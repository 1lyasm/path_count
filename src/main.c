...
void take_n(int *n) {
  printf("Enter n (row and column count of the matrix): ");
  scanf("%d", n);
}

void read_matrix(int mat[256][256], int n) {
  int i, j;
  printf("Enter %d x %d matrix (top left must be 1 and bottom right must be 0): \n", n, n);
  for (i = 0; i < n; ++i) {
      printf("Row %d: ", i);
    for (j = 0; j < n; ++j) {
      scanf("%d", &mat[i][j]);
...
int main() {
  ...
  printf("\nTotal path count from top left to bottom right: %d\n", path_count);
